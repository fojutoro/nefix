...
// Home and Unfiled are not classes but are chosen from the rail like one, so
// the whole rail is a single union rather than a class id plus two flags.
...
// because membership is notebookId. Home's shelf is every note, so that a
// recent opened from it is found and Back returns to Home.
export function scopeOf(
  ...
  // it appears in the class's contents, on Home's shelf and in every search —
  // three lists where a page torn out of its book is wrong. The guard sits
  // here because those three lists share this predicate and nothing else. A
