export const en = {
  'app.title': 'nefix',
  'app.tagline': 'Offline-first study notes.',
  'app.language': 'Language',
  'app.switchLanguage': 'Switch to Slovak',
  'sync.offline': 'Offline. Notes are saved on this device.',
  'sync.syncing': 'Syncing…',
  'sync.signIn': 'Sign in to sync.',
  'sync.error': 'Sync failed. It will try again.',
  'sync.never': 'Not synced yet.',
  'sync.lastSynced': 'Synced {{when}}.',
  // Suffixed onto the copy kept when the server rejects a stale push.
  'sync.olderVersion': '(older version)',
  // The dot's accessible name. Colour alone is not a signal for everyone, so
  // the state goes in the name whatever the fill says.
  'sync.dot': 'Sync: {{state}}',
  'sync.state.synced': 'up to date',
  'sync.state.syncing': 'syncing',
  'sync.state.pending_one': '{{count}} change waiting',
  'sync.state.pending_other': '{{count}} changes waiting',
  'sync.state.offline': 'offline',
  'sync.state.failed': 'failed',
  'sync.state.signedOut': 'signed out',
  'sync.account': 'Account and sync',
  'sync.stateLabel': 'State',
  'sync.lastSync': 'Last synced',
  'sync.debug': 'Sync diagnostics',
  'sync.cursor': 'Cursor',
  'sync.dexie': 'Database version',
  'sync.build': 'Server build',
  'sync.table': 'Table',
  'sync.table.classes': 'Classes',
  'sync.table.notebooks': 'Notebooks',
  'sync.table.notes': 'Notes',
  'sync.rows': 'Rows',
  'sync.dirty': 'Waiting',
  'sync.compare': 'Compare with server',
  'sync.comparing': 'Comparing…',
  'sync.serverRows': 'Rows on the server',
  'sync.localRows': 'Rows on this device',
  'sync.onlyServer': 'Only on the server',
  'sync.onlyLocal': 'Only on this device',
  'sync.versionDiffers': 'Different versions',
  'sync.noCycles': 'No sync has run yet on this device.',
  'sync.pushLine':
    'push {{sent}} → {{accepted}} ok, {{conflicted}} conflict, {{forbidden}} refused',
  'sync.pullLine': 'pull {{got}} → {{applied}} applied, cursor {{before}} → {{after}}',
  // The notebook created with a class, so there is somewhere to type at
  // once. Renameable afterwards, so this is a starting point, not a name.
  'notebooks.general': 'General',
  'rail.label': 'Classes',
  // A pseudo-class, and first, because most of the time what you want is
  // something from the last few hours.
  'rail.today': 'Today',
  'rail.todayEmpty': 'Nothing yet today',
  'rail.unfiled': 'Unfiled',
  'rail.archived': 'Archived',
  'rail.newClass': '+ New class',
  'rail.className': 'Class name',
  'classes.empty': 'A class is where the notes for one subject live.',
  'classes.addFirst': 'Add the first class',
  'class.activity': 'Weeks written in',
  'state.none': 'Nothing here yet',
  'state.recent': 'Last written {{when}}',
  'state.idle': 'Nothing here for {{when}}',
  'rail.resize': 'Resize the rail',
  'editor.back': 'Back to {{name}}',
  'editor.bold': 'Bold',
  'editor.italic': 'Italic',
  'editor.strike': 'Strikethrough',
  'editor.code': 'Code',
  'editor.paragraph': 'Paragraph',
  'editor.heading': 'Heading {{level}}',
  'editor.link': 'Link',
  'editor.linkUrl': 'Link URL',
  'editor.math': 'Formula',
  'editor.insertBlock': 'Insert block',
  'block.bulletList': 'Bulleted list',
  'block.orderedList': 'Numbered list',
  'block.taskList': 'Task list',
  'block.quote': 'Quote',
  'block.codeBlock': 'Code block',
  'block.table': 'Table',
  'block.divider': 'Divider',
  'block.math': 'Display formula',
  'class.rename': 'Rename class',
  'class.code': 'Course code',
  'class.addCode': 'Add code',
  'class.menu': 'Class actions',
  'class.colour': 'Colour',
  'class.colourOption': 'Colour {{n}}',
  'class.semester': 'Semester',
  'class.addSemester': 'Add semester',
  'class.archive': 'Archive',
  'class.archiveConfirm':
    'Archive {{name}}? Its notes stay, under Archived.',
  'class.delete': 'Delete',
  'class.deleteConfirmEmpty': 'Delete {{name}}? This cannot be undone.',
  'class.deleteConfirm_one':
    'Delete {{name}} and its {{count}} note? This cannot be undone.',
  'class.deleteConfirm_other':
    'Delete {{name}} and its {{count}} notes? This cannot be undone.',
  'class.deleteConfirmDeadlines_one':
    'Delete {{name}} and its {{count}} deadline? This cannot be undone.',
  'class.deleteConfirmDeadlines_other':
    'Delete {{name}} and its {{count}} deadlines? This cannot be undone.',
  // Two counts in one sentence, so neither can be the key's own {{count}}:
  // i18next pluralises on one number at a time. The halves are pluralised
  // separately through notes.count and deadlines.count and arrive here already
  // written, which is also what keeps the Slovak forms independent.
  'class.deleteConfirmBoth':
    'Delete {{name}}, {{notes}} and {{deadlines}}? This cannot be undone.',
  'search.label': 'Search notes',
  'search.placeholder': 'Search',
  'search.clear': 'Clear search',
  'search.matches_one': '{{count}} match',
  'search.matches_other': '{{count}} matches',
  'auth.signInHeading': 'Sign in',
  'auth.registerHeading': 'Create an account',
  'auth.email': 'Email',
  'auth.password': 'Password',
  'auth.username': 'Username',
  'auth.displayName': 'Display name',
  'auth.submitSignIn': 'Sign in',
  'auth.submitRegister': 'Create account',
  'auth.working': 'Working…',
  'auth.toRegister': 'No account? Create one.',
  'auth.toSignIn': 'Already have an account? Sign in.',
  'auth.emailRule': 'Email must contain exactly one @ with text either side.',
  'auth.passwordRule': 'Password must be 8 to 128 bytes.',
  'auth.usernameRule':
    'Username must be 3 to 32 characters, using only a-z, 0-9, _ and -.',
  'auth.displayNameRule': 'Display name must be 1 to 64 characters.',
  // The server never says which of the two collided, and neither may this.
  'auth.taken': 'That username or email is already taken.',
  'auth.offline': 'Cannot reach the server. Check your connection.',
  'auth.failed': 'Something went wrong. Please try again.',
  'auth.signOut': 'Sign out',
  'auth.signOutConfirm':
    'Sign out? The notes on this device will be deleted.',
  // Changes rather than notes: the count covers classes and notebooks too,
  // and all three are deleted by the sign-out this confirms.
  'auth.signOutConfirmDirty_one':
    '{{count}} change has not synced yet and will be lost. Sign out anyway?',
  'auth.signOutConfirmDirty_other':
    '{{count}} changes have not synced yet and will be lost. Sign out anyway?',
  'auth.sessionEnded': 'Your session ended. Sign in again to keep syncing.',
  'auth.signInAgain': 'Sign in again',
  'notes.listLabel': 'Notes',
  'notes.editorLabel': 'Note body, Markdown',
  'notes.mathLabel': 'Formula, LaTeX',
  'notes.create': 'New note',
  'notes.untitled': 'Untitled',
  'notes.delete': 'Delete {{title}}',
  'notes.deleteConfirm': 'Delete “{{title}}”? This cannot be undone.',
  'notes.count_one': '{{count}} note',
  'notes.count_other': '{{count}} notes',
  'deadlines.count_one': '{{count}} deadline',
  'deadlines.count_other': '{{count}} deadlines',
  'deadlines.label': 'Deadlines',
  'deadlines.new': 'New deadline',
  'deadlines.create': '+ New deadline',
  'deadlines.next': 'Next deadline',
  'deadlines.upcoming_one': '{{count}} upcoming',
  'deadlines.upcoming_other': '{{count}} upcoming',
  'deadlines.showAll': 'Show done and overdue ({{count}})',
  'deadlines.showUpcoming': 'Upcoming only',
  // A span already written by Intl — "2 days" — and never a relative time:
  // "2 days ago" describes a date, and this is a miss.
  'deadlines.late': '{{when}} late',
  'deadlines.done': 'Mark {{title}} done',
  'deadlines.class': 'Class',
  'deadlines.noClass': 'No class',
  'deadlines.title': 'Title',
  'deadlines.due': 'Due date',
  'deadlines.kind': 'Kind',
  'deadlines.kind.test': 'Test',
  'deadlines.kind.assignment': 'Assignment',
  'deadlines.kind.other': 'Other',
  'deadlines.note': 'Note',
  'deadlines.topics': 'Topics',
  'deadlines.addTopic': 'Add “{{text}}” as a topic',
  'deadlines.removeTopic': 'Remove {{heading}}',
  'deadlines.unwritten': 'Not in your notes yet',
  'deadlines.save': 'Add deadline',
  'deadlines.cancel': 'Cancel',
  'deadlines.topicMissing': '“{{heading}}” is no longer in this note.',
  'deadlines.noteGone': 'The note with “{{heading}}” has been deleted.',
  'notes.moreHeadings_one': '+{{count}} more',
  'notes.moreHeadings_other': '+{{count}} more',
  'notes.emptySearch': 'No notes match your search.',
  // A collegebook is where a lecture is dumped as it happens; a note is one
  // topic in depth. The two words have to stay distinct in the interface,
  // because choosing between them is the whole point.
  'books.label': 'Collegebooks',
  'books.create': '+ New collegebook',
  'books.name': 'Collegebook name',
  'books.pages_one': '{{count}} page',
  'books.pages_other': '{{count}} pages',
  'book.pagesLabel': 'Pages',
  'book.newPage': '+ New page',
  'book.settings': 'Page appearance',
  // One collegebook, not one page: a notebook is ruled or plain, it is not
  // ruled on page six.
  'book.ruling': 'Ruling',
  'book.ruling.none': 'Plain',
  'book.ruling.ruled': 'Ruled',
  // What maths students actually use, which is the reason it is here.
  'book.ruling.squared': 'Squared',
  'book.pitch': 'Line pitch',
  'book.marginRule': 'Margin rule',
  'book.inset': 'Inset',
  'book.page': 'Page',
  'book.pageNumbers': 'Page numbers',
  'book.pageHeight': 'Page height',
  'book.lines_one': '{{count}} line',
  'book.lines_other': '{{count}} lines',
  'book.paper': 'Paper',
  'book.paperHex': 'Paper hex',
  'book.paperSwatch': 'Paper swatch',
  'book.paperOption': 'Paper {{n}}',
  'book.grain': 'Grain',
  'book.grainStrength': 'Grain strength',
  'book.text': 'Text',
  'book.textSize': 'Text size',
  'book.ink': 'Text colour',
  'book.inkHex': 'Text hex',
  'book.inkSwatch': 'Text swatch',
  'book.inkOption': 'Text colour {{n}}',
  'book.font': 'Font',
  'book.font.sans': 'Sans',
  'book.font.serif': 'Serif',
  'book.font.mono': 'Monospace',
  // Said rather than enforced: a faint watermark is somebody's idea of a
  // good time, and this is not the place to argue.
  'book.lowContrast':
    'This text may be hard to read on this paper ({{ratio}}:1).',
  'book.reset': 'Reset to defaults',
  'book.resetConfirm': 'Reset this collegebook to the default appearance?',
}
